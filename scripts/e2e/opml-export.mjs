...
export async function runOpmlExportE2e() {
  ...
  const opmlPath = path.join(fs.mkdtempSync(path.join(os.tmpdir(), "kiji-e2e-opml-")), "export-source.opml");
  ...
  try {
    ...
    const opmlText = readHarnessExport(e2eDir);
    if (!opmlText || !opmlText.includes('type="rss"')) {
      throw new Error("Expected exported OPML file with RSS outlines");
    }

    return {
      skipped: false,
      outlineCount: exported.payload?.outlineCount ?? 0,
      byteLength: exported.payload?.byteLength ?? 0,
    };
  } catch (error) {
...
if (isMainModule) {
  runOpmlExportE2e()
    .then((result) => {
      ...
      if (result.skipped) {
        console.log(`[e2e:opml-export] Skipped: ${result.reason}`);
        process.exit(0);
      }
      console.log(`[e2e:opml-export] Passed: outlines=${result.outlineCount} bytes=${result.byteLength}`);
      process.exit(0);
    })
    .catch((error) => {
      console.error(`[e2e:opml-export] Failed: ${error instanceof Error ? error.message : error}`);
      process.exit(1);
    });
}
